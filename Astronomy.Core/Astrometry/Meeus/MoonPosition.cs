...
namespace Astronomy.Core.Astrometry.Meeus
{
    ...
    internal static class MoonPosition
    {
        ...
        // Columns: D, M, M', F, sigma_l (in 0.000001 deg), sigma_r (in 0.001 km).
        ...
        // Columns: D, M, M', F, sigma_b (in 0.000001 deg).
        ...
        public static (double LonDeg, double LatDeg, double DistanceKm) ApparentEcliptic(double jd)
        {
            ...
            double sigmaL = 0.0;
            double sigmaR = 0.0;
            int rowsLR = mTermsLR.GetLength(0);
            for (int i = 0; i < rowsLR; i++)
            {
                int dC  = mTermsLR[i, 0];
                int mC  = mTermsLR[i, 1];
                int mpC = mTermsLR[i, 2];
                int fC  = mTermsLR[i, 3];
                double lC = mTermsLR[i, 4];
                double rC = mTermsLR[i, 5];
