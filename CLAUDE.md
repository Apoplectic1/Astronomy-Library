...
- **`Astronomy.Core.Tests`** — `net10.0-windows` x64, `OutputType=Exe`. Hosts both xUnit tests (`Tests/`) and BenchmarkDotNet benchmarks (`Benchmarks/`) in a single assembly. `GenerateProgramFile=false` because `Program.cs` defines its own `Main` that delegates to `BenchmarkSwitcher.FromAssembly(...)`. References both `Astronomy.Core` and `Astronomy.PCL`. `xunit.runner.visualstudio` is intentionally on the `4.0.0-pre.X` prerelease line — VS2026 IDE test discovery and the .NET 10 SDK both need the 4.x architecture, but the stable channel hasn't released 4.x yet (stable is still on the older 3.x line, which would be a downgrade from `4.0.0-pre` not an upgrade).
...
- **`Astronomy.NINA`** — `net10.0-windows` x64, `LangVersion latest`, `Nullable enable`, `ImplicitUsings enable`. NINA-integration / planning library. Phase A surface (2026-05-18) is the **image-library scanner**: header-only XISF reader (`Xisf/XisfHeaderReader.cs`, ported from `XisfFileManager/Files/XisfXmlReader.cs` — pure managed `XDocument.Parse()`, no native dep), typed FITS accessors (`Xisf/XisfHeader.cs`, modeled on XFM `KeywordList`), directory walker + per-target / per-filter aggregator (`Xisf/ImageLibraryScanner.cs`) producing `ImageLibraryReport` of `TargetReport[]` × `FilterAggregate[]`. ProjectReference to Astronomy.Core only — no NINA assembly dependency yet (deferred to Phase D when the `InputTargetAdapter` lands). `<InternalsVisibleTo Include="Astronomy.NINA.Tests" />` exposes the `ImageLibraryScanner` static helpers (`ParseFilterDirName`, `NormalizeFilterName`) to tests. Forward roadmap (Phases B-D in `~/.claude/plans/what-is-next-from-crispy-garden.md`): rich `Target` class wrapping `Astronomy.Core.Targets.Target` + composed `FilterHistory` / `Filter` / `ExposureSettings` / `PlannedExposure`; then TP migration; then NINA-side `InputTargetAdapter`.
- **`Astronomy.NINA.Tests`** — `net10.0-windows` x64, `OutputType=Exe`. xUnit tests for Astronomy.NINA. Includes a `[Fact]`-tagged smoke test `Xisf/ImageLibrarySmokeTest.cs` that scans a real library when `TP_SMOKE_IMAGE_LIBRARY` env var is set (silent no-op otherwise). Separate from `Astronomy.Core.Tests` because Astronomy.NINA will eventually pull NINA assemblies that have no business in Core's test graph.
...
# Astronomy.NINA build + tests — pure-managed too (no native dep in Phase A)
