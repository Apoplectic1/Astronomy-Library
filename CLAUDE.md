...
- **`Astronomy.NINA`** — `net10.0-windows` x64, `LangVersion latest`, `Nullable enable`, `ImplicitUsings enable`. NINA-integration / planning library. ProjectReference to Astronomy.Core only — no NINA assembly dependency yet (deferred to Phase D when `InputTargetAdapter` lands). `<InternalsVisibleTo Include="Astronomy.NINA.Tests" />` exposes scanner internals to tests. Current surface (Phases A + B, 2026-05-18):
  - **Phase A — image-library scanner** (`Xisf/` subnamespace): header-only XISF reader (`XisfHeaderReader.cs`, ported from `XisfFileManager/Files/XisfXmlReader.cs` — pure managed `XDocument.Parse()`, no native dep), typed FITS accessors (`XisfHeader.cs`, modeled on XFM `KeywordList`, surfaces value + comment for each keyword), directory walker + per-target / per-filter aggregator (`ImageLibraryScanner.cs`) producing `ImageLibraryReport` of `TargetReport[]` × `FilterAggregate[]`.
