...
- **`Astronomy.Catalog`** — `net10.0-windows`, `LangVersion latest`, `Nullable enable`, `ImplicitUsings enable`. The shared catalog-database library **and** the owner of the .xisf-library scanner. One `Catalog.db` (SQLite via `Microsoft.Data.Sqlite`) holds two planes: the **plan** (profile/project/target/exposure_template/exposure_plan, cleaned up from N.I.N.A. Target Scheduler; goals = `exposure_plan.desired_count`) and the disk-derived **inventory** (`inventory_target` + `inventory_filter`, the persisted aggregates of a library scan). `Scan/` holds the moved `ImageLibraryScanner` + report types (depends on `Astronomy.XISF`). `Schema/` holds the POCOs/mappers + `schema.sql`; `SchemaManager` applies it idempotently — **no migration framework**: the catalog is fully derived (scan + TS import) and rebuildable, so a schema change just means deleting `Catalog.db`. `CatalogStore` is the read/write surface (`ReplaceInventory(report)` persists a scan transactionally); `TargetScheduler/TargetSchedulerReader` reads TS's `schedulerdb.sqlite` read-only (Mode=ReadOnly + busy-timeout, explicit columns). Consumed by XFM / TP / IS / ISP and the TargetCatalogManager (TCM) app. Pure-managed → builds with `dotnet build`.
- **`Astronomy.Catalog.Tests`** — `net10.0-windows` x64, `OutputType=Exe`. xUnit tests for Astronomy.Catalog: the moved scanner tests (`Scan/`), schema creation (WAL, lookups), plan + inventory round-trips, the scan→`ReplaceInventory`→read path, and a read of the pinned TS snapshot. 28 tests.
