...
- **`Astronomy.NINA`** — `net10.0-windows` x64, `LangVersion latest`, `Nullable enable`, `ImplicitUsings enable`. NINA-integration / planning library. ProjectReference to Astronomy.Core + Astronomy.XISF — no NINA assembly dependency yet (deferred to Phase D when `InputTargetAdapter` lands). `<InternalsVisibleTo Include="Astronomy.NINA.Tests" />` exposes scanner internals to tests. Current surface (Phases A + B, 2026-05-18):
  - **Phase A — image-library scanner** (`Xisf/` subnamespace): directory walker + per-target / per-filter aggregator (`ImageLibraryScanner.cs`) over an `Astronomy.XISF.XisfHeader` stream, producing `ImageLibraryReport` of `TargetReport[]` × `FilterAggregate[]`. The XISF reading primitives themselves moved to Astronomy.XISF.
...
- **`Astronomy.NINA.Tests`** — `net10.0-windows` x64, `OutputType=Exe`. xUnit tests for Astronomy.NINA (scanner, composition types, adapter). Includes a `[Fact]`-tagged smoke test `Xisf/ImageLibrarySmokeTest.cs` that scans a real library when `TP_SMOKE_IMAGE_LIBRARY` env var is set (silent no-op otherwise). Separate from `Astronomy.Core.Tests` because Astronomy.NINA will eventually pull NINA assemblies that have no business in Core's test graph.
