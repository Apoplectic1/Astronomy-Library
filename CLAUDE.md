...
- **DateTime kinds.** `Location.DateTime` is caller-owned. `Local`/`Unspecified` are treated as local; `Utc` is no-op'd. `NightWindow` outputs are `Kind=Utc`. The canonical helper for normalising arbitrary `DateTime` input at boundary surfaces is `Astronomy.Core.Time.TimeKindGuard.AsUtc(DateTime)` (internal); `AltAzCalculator.Of` and the Sun-family / `AstroUtil` / `NightCalculator` helpers all route through it as of 2026-05-18. One deliberate exception: `LunarAge.DaysAt` throws on non-Utc rather than converting, because it sits inside `BestSession.MoonClearIntersect`'s tight loop where a stray non-Utc kind would silently corrupt the whole-night sweep.
