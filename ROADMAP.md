...
- **2026-08-13** — **Plan-plane write surface** (openspec `add-plan-write-surface`, seeded by ISM's `add-planner-surface`): `PlanWriter` over an open `IntentStore` — `UpsertPlan` (full-value, `created_at` create-only), `ReplaceIntervals` (whole-set ordered replace, foreign-plan interval loud), `FindCurrentPlan` (single non-superseded per profile+night, duplicate loud), `ReadIntervals`; `PlanIntent`/`PlanIntervalIntent` schema-mirror records; caller-owned transactions throughout. No migration. Catalog suite 323 green.
