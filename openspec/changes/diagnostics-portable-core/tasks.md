...
- [x] 7.4 Manual verification (feature-correct, not just code-correct): Ctrl+N flow in TSM and TP — open, capture, delayed capture, checkpoint OK, cancel/close-X terminator, singleton re-invoke — VERIFIED by user in-app 2026-08-10 (tsm.log ids 1805/e3ff/2e43/ef4d, tp.log id 74d1): TSM open+END over the AppDialog-hook path with a live modal in-shot, flyout captured in the END shot, CANCEL terminator, singleton (second Ctrl+N → no second START); TP delayed capture containing the Edit Filters dialog. Real EDIT→PULL→PUSH cycle healthy on the new build.
